...
export default function MaterialPage3() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const mockContent = {
      id: "3",
      filename: "ejercicios-parciales.pdf",
      type: "pdf",
      extracted: true,
      content: "Contenido simulado de ejercicios parciales - problemas tipo examen con soluciones paso a paso. Cubre los temas fundamentales del primer año."
    };
    if (mockContent) {
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold">ejercicios-parciales.pdf</h1>
              <span className="rounded-full px-2.5 py-0.5 text-xs font-medium bg-primary/10 text-primary">pdf</span>
            </div>
            <p className="text-sm text-[var(--muted-foreground)]">ID: 3 • Nivel: Primer año</p>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-2xl p-6 rounded-2xl border border-[var(--border)] bg-[var(--card)]">
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <p className="text-sm text-[var(--muted-foreground)]">
              Contenido extraído (modo demo) - el archivo original está en Google Drive y será procesado cuando esté disponible. Este material corresponde a ejercicios tipo parcial.
            </p>
          </div>
        </div>
      </main>
