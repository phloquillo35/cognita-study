...
export default function MaterialPage1() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const mockContent = {
      id: "1",
      filename: "manual-matematica.pdf",
      type: "pdf",
      extracted: true,
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold">manual-matematica.pdf</h1>
              <span className="rounded-full px-2.5 py-0.5 text-xs font-medium bg-primary/10 text-primary">pdf</span>
            </div>
            <p className="text-sm text-[var(--muted-foreground)]">ID: 1 • Nivel: Primer año</p>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-2xl p-6 rounded-2xl border border-[var(--border)] bg-[var(--card)]">
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <p className="text-sm text-[var(--muted-foreground)]">
              Contenido extraído (modo demo) - el archivo original está en Google Drive y será procesado cuando esté disponible. Este material corresponde a la materia AM I (Matemática I).
            </p>
          </div>
        </div>
      </main>
